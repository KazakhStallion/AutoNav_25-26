...
add_once "[ -f source /opt/ros/humble/setup.bash] && source /opt/ros/humble/setup.bash"
add_once "[ -f source /autonav/isaac_ros-dev/install/setup.bash ] && source /autonav/isaac_ros-dev/install/setup.bash"
